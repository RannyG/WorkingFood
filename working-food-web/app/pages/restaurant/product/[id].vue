...
const { data } = await useFetch(config.public.apiUrl + `/product/${id.value}`, {
  headers: {
...
<template>
  <h2 class="font-bold text-3xl">Adicionar produto</h2>
  <hr />
  ...
  <form class="flex flex-col gap-4 mt-5" @submit.prevent="onSubmit">
    ...
    <div>
      <label class="label">
        Preço
        <input class="input" type="number" v-model="form.price" />
      </label>
